...
@pytest.mark.parametrize("mode", MODE_LIST)
@pytest.mark.parametrize("ieee", [False, True])
def test_reinterpret(eb, mb, mode, ieee):
    FT = FPVector[eb, mb, mode, ieee]
    #basic sanity
...
def test_reinterpret_pyrandom(eb, mb, mode, ieee, mean, variance):
    FT = FPVector[eb, mb, mode, ieee]
    for _ in range(NTESTS):
    ...
    for _ in range(NTESTS):
        ...
        if ieee:
            if not f.fp_is_NaN():
                ...
                assert bv1[mb:-1] == BitVector[eb](-1)
                assert bv2[mb:-1] == BitVector[eb](-1)
            ...
            if bv1[mb:-1] == 0:
                assert f.fp_is_zero()
            elif bv1[mb:-1] == BitVector[eb](-1):
                assert f.fp_is_infinite()
            else:
                assert bv1 == bv2
